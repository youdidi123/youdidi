...
func (c *Client) postWithoutCert(url string, params Params) (string, error) {
    ...
    logs.Debug("postWithoutCert:%s", p)
    logs.Debug("postWithoutCert:%s", MapToXml(p))
...
func (c *Client) ProcessResponseXmlEnp(xmlStr string) (Params, error) {
	var returnCode string
	logs.Debug("WeiXin return Xml is :%s ", xmlStr)
	params := XmlToMap(xmlStr)
	if params.ContainsKey("return_code") {
...
func (c *Client) EnpTransfers(params Params) (Params, error) {
	...
	return c.ProcessResponseXmlEnp(xmlStr)
}
